...
let StorageService = class StorageService {
    ...
    async uploadBuffer(buffer, path, contentType) {
        await this.client.putObject(this.bucket, path, buffer, buffer.length, {
            'Content-Type': contentType,
        });
        return this.getFileUrl(path);
    }
    async uploadFromUrl(sourceUrl, path) {
    ...
    generatePath(projectId, category, extension) {
        return `projects/${projectId}/${category}/${(0, uuid_1.v4)()}.${extension}`;
    }
};
