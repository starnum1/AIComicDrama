...
let StoryboardService = StoryboardService_1 = class StoryboardService {
    ...
    async generateForEpisode(projectId, episode, characters, scenes, llmConfig) {
        ...
        for (const shot of result.shots) {
            ...
            const createdShot = await this.prisma.shot.create({
                data: {
                    ...
                    dialogue: shot.dialogue ?? client_1.Prisma.JsonNull,
                    narration: shot.narration ?? client_1.Prisma.JsonNull,
                    sfx: shot.sfx,
        ...
        this.logger.log(`Episode ${episode.episodeNumber} - 生成 ${result.shots.length} 个镜头`);
...
严格JSON格式：
{
  "shots": [
    ...
    buildUserPrompt(episode, characters, scenes) {
