...
let AnalysisService = AnalysisService_1 = class AnalysisService {
    ...
    async execute(projectId) {
        ...
        for (const ep of episodeResult.episodes) {
            const originalText = this.extractOriginalText(novel.originalText, ep.text_start_marker, ep.text_end_marker);
        ...
        this.logger.log(`Project ${projectId} - Phase 2 完成：${episodeResult.episodes.length}集`);
    }
...

{
  "episodes": [
    ...
    buildEpisodeUserPrompt(novelText, characterNames, sceneNames) {
        return `## 已提取的角色列表
...
注意：不要复制原文，只需提供 text_start_marker 和 text_end_marker 标记每集的原文范围。`;
    ...
    extractOriginalText(fullText, startMarker, endMarker) {
        const startIdx = this.fuzzyIndexOf(fullText, startMarker);
        const endIdx = this.fuzzyIndexOf(fullText, endMarker);
        if (startIdx === -1 || endIdx === -1) {
            this.logger.warn(`原文标记匹配失败: start="${startMarker.slice(0, 10)}..." (${startIdx}), ` +
